...
Ub    = 1     # Bulk velocity
Re    = rho * Ub * L / mu # Reynolds Number
P_str = 1e6 # Pressure at the beginning of the pipe
P_end = 0   # Pressure at the end of the pipe
P_x = P_end - P_str # Pressure (adimensional) difference
...
def PDE_MOM(k):
    ...
    with ns.GradientTape(persistent=True) as tape:
        ...
        unnormed_lhs = rho * (u_vect[:,0] * deqx + u_vect[:,1] * deqy) - mu * (lapl_eq) + dp 
...
BCN_losses = [LMS('BCN_u_x1', lambda: BC_N("DX",0,0), weight = 1e0),
              LMS('BCN_v_x1', lambda: BC_N("DX",1,0), weight = 1e0)]
